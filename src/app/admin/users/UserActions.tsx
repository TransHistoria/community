...
export function UserActions({ userId, currentTier, status, onUpdated }: { userId: string; currentTier: string; status: string; onUpdated?: () => void }) {
  ...
  async function applyTier() {
    ...
    setPending(true);
    const res = await api.admin.setUserStatus(userId, tier);
    setPending(false);
    if (res.ok) { toast({ title: "已更新", variant: "success" }); onUpdated?.(); }
    else { toast({ title: "失败", variant: "danger" }); }
  }
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <Button size="sm" variant="outline" disabled={pending} onClick={applyTier}>提级</Button>
      {status === "ACTIVE" ? (
        <Button size="sm" variant="destructive" disabled={pending} onClick={async () => {
