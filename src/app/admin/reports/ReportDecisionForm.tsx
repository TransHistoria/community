...
const HIDE_LABEL: Record<string, string> = { USER: "封禁该用户", EVENT: "取消该活动", COMMENT: "隐藏该评论" };

export function ReportDecisionForm({ reportId, targetType, onDecided }: { reportId: string; targetType: "USER" | "EVENT" | "COMMENT"; onDecided?: () => void }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" disabled={pending} onClick={() => decide("DISMISSED")}>驳回（无问题）</Button>
        <Button disabled={pending} onClick={() => decide("RESOLVED")}>处理（属实）</Button>
      </div>
    </div>
  );
}
...
export function ReportDecisionForm({
  ...
  async function decide(decision: "RESOLVED" | "DISMISSED") {
    setPending(true);
    const res = await resolveReport(reportId, decision, note || undefined, hide);
    setPending(false);
    if (res.ok) {
      toast({ title: "已处理", variant: "success" });
      router.refresh();
