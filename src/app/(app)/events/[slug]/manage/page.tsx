...
export default function ManageEventPage() {
  ...
  function loadData() {
    if (!slug) return;
    api.events.get(slug).then(({ event: e }) => {
      const ev = e as ApiEvent;
      setEvent(ev);
      api.events.listRegistrations(ev.id).then(({ registrations: rs }) => {
        setRegs(rs as ApiReg[]);
      });
    });
  }
  ...
  if (!canEditEvent(user, { ...event, startAt: new Date(), endAt: new Date(), organizerId: event.organizer_id })) {
                          ...
                          return (
                            <div className="text-xs text-ink-muted bg-bg-warm rounded-md p-3 space-y-1">
                              {Object.entries(ans).map(([key, v]) => (
                                <div key={key}><strong>{key}:</strong> {v}</div>
                              ))}
                            </div>
                          );
                        } catch { return null; }
